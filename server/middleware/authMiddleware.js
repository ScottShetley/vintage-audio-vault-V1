...
const protect = async (req, res, next) => {
  ...
  if (
    ...
    try {
      ...
      // Verify token
      const decoded = jwt.verify(token, process.env.JWT_SECRET || 'your-super-secret-key-for-dev');
      ...
      req.user = await User.findById(decoded.id).select('-password -passwordResetToken -passwordResetExpires -verificationToken');
